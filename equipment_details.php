...
$booking_count_query = "SELECT COUNT(*) as total_bookings 
                       ...
                       WHERE equipment_id = ? AND status = 'COM'";
...
<div class="container" style="margin-top: 40px; margin-bottom: 40px;">
    ...
    <!-- ADD THIS TO equipment_details.php or product_details.php -->

<?php
// Handle complaint submission
if (isset($_POST['submit_complaint'])) {
    $complaint_type = $_POST['complaint_type']; // 'E' for equipment, 'P' for product
    $item_id = intval($_POST['item_id']);
    $description = trim($_POST['description']);
    $user_id = $_SESSION['user_id'] ?? 0;
    
    if (!empty($description) && $user_id > 0) {
        $insert_sql = "INSERT INTO complaints (User_id, Complaint_type, ID, Description, Status) 
                       VALUES (?, ?, ?, ?, 'O')";
        $insert_stmt = $conn->prepare($insert_sql);
        if ($insert_stmt) {
            $insert_stmt->bind_param('isis', $user_id, $complaint_type, $item_id, $description);
