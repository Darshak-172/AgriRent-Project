...
if (isset($_POST['add_subcategory'])) {
    ...
    if ($stmt->execute()) {
        $message = "Equipment subcategory added successfully!";
    } else {
...
if (isset($_POST['edit_subcategory'])) {
    ...
    if ($stmt->execute()) {
        $message = "Equipment subcategory updated successfully!";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($check->fetch_assoc()['count'] > 0) {
        $error = "Cannot delete subcategory with existing equipment!";
    } else {
        $conn->query("DELETE FROM equipment_subcategories WHERE Subcategory_id = $id");
        $message = "Equipment subcategory deleted successfully!";
    }
}
...
if (isset($_GET['edit'])) {
    ...
    $edit_result = $conn->query("SELECT * FROM equipment_subcategories WHERE Subcategory_id = $edit_id");
    $edit_subcategory = $edit_result->fetch_assoc();
}

// Get all subcategories for this category
$subcategories = $conn->query("SELECT * FROM equipment_subcategories WHERE Category_id = $category_id ORDER BY Subcategory_name DESC");
...
<div class="main-content">
    <div class="breadcrumb">
        <a href="categories.php"> Equipment Categories</a> 
    </div>
    
    <h1> <?= htmlspecialchars($category['Name']) ?> - Subcategories</h1>
    ...
    <div class="form-box">
        <h3><?= $edit_subcategory ? 'Edit Equipment Subcategory' : 'Add New Equipment Subcategory' ?></h3>
        <form method="POST">
            ...
            <?php if ($edit_subcategory): ?>
                ...
                <a href="subcategories.php?category_id=<?= $category_id ?>" class="btn btn-cancel">Cancel</a>
            <?php else: ?>
    ...
    <!-- Subcategories List -->
    <h3>All <?= htmlspecialchars($category['Name']) ?> Subcategories</h3>
...
/* Action buttons - Professional with hover effects */
.btn-action {
    ...
    $('#subcategorySearch').on('keyup', function() {
        ...
        if (visibleRows === 0 && searchTerm !== '') {
            if ($('#noResultsRow').length === 0) {
                $('#subcategoriesTable').append('<tr id="noResultsRow"><td colspan="4" style="text-align: center; padding: 20px; font-style: italic;">No equipment subcategories found matching your search</td></tr>');
            }
        } else {
            $('#noResultsRow').remove();
        }
