...
$products = $conn->query("
    SELECT p.*, u.Name as seller_name, u.Email as seller_email, u.Phone as seller_phone, i.image_url,
           c.Category_name as category_name,
           sc.Subcategory_name as subcategory_name,
           CONCAT(ua.address, ', ', ua.city, ', ', ua.state, ' - ', ua.Pin_code) AS location
    FROM product p
    ...
    LEFT JOIN product_categories c ON sc.Category_id = c.Category_id
        ...
        <tbody>
            <?php if ($products->num_rows > 0): ?>
                <?php while ($product = $products->fetch_assoc()): ?>
                    ...
                    $unitDisplay = '';
                    switch (strtoupper(trim($product['Unit']))) {
                        case 'K':
                            $unitDisplay = 'kg';
                            break;
                        case 'L':
                    ...
                    <tr class="product-row">
                        ...
                        <td><?= $product['Quantity'] ?> <?= $unitDisplay ?></td>
...
<script>
// CORRECTED UNIT CONVERSION HELPER FUNCTION
        function getUnitDisplay(unit) {
            switch (String(unit).toUpperCase().trim()) {
                case 'K':
                    return 'Kilogram (kg)';
                case 'L':
                    return 'Liter';
            ...
            $('#liveSearch').on('keyup', function () {
                ...
                if (visibleRows === 0 && searchTerm !== '') {
                    if ($('#noResultsRow').length === 0) {
                        $('#productsTable tbody').append('<tr id="noResultsRow"><td colspan="7" style="text-align: center; padding: 20px; font-style: italic;">No products found </td></tr>');
                    }
                } else {
                    $('#noResultsRow').remove();
                }
...
// View product details function
        function viewProduct(product) {
            ...
            document.getElementById('detailUnit').textContent = getUnitDisplay(product.Unit);
