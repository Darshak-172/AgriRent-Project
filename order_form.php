...
//--- CALCULATE AVAILABLE QUANTITY FOR CON STATUS ---//
$available_quantity = $product['Quantity'];
$is_out_of_stock = false;

if ($product['Approval_status'] === 'CON') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (!empty($_POST['quantity']) || isset($_POST['place_order']))) {
    ...
    if ($quantity > $available_quantity) {
        $errors[] = "Only {$available_quantity} {$product['Unit']} available in stock";
    }
    ...
    if (empty($errors)) {
        ...
        if ($stmt) {
            ...
            $current_available = $available_quantity - $total_pending;

            if ($quantity > $current_available) {
                $errors[] = "Only {$current_available} {$product['Unit']} currently available (pending orders)";
            }
        }
    }
...
<div class="container" style="margin-top: 40px; margin-bottom: 40px;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 400px; gap: 40px;">
        ...
        <div>
            ...
            <!-- Out of Stock Alert (when showing form) -->
            <?php if ($is_out_of_stock): ?>
                <div style="background: #f8d7da; color: #721c24; padding: 20px; border-radius: 8px; margin-bottom: 30px; border-left: 6px solid #dc3545; display: flex; align-items: center; gap: 15px;">
                    <div style="font-size: 32px;">⚠️</div>
                    <div>
                        <h3 style="margin: 0 0 5px 0; color: #721c24;">Out of Stock</h3>
                        <p style="margin: 0; font-size: 14px;">This product is currently unavailable.</p>
                    </div>
                </div>
            <?php endif; ?>
                ...
                <div style="display: flex; gap: 20px; align-items: start;">
                    ...
                    <div style="flex: 1;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; margin-bottom: 15px;">
                            ...
                            <div>
                                <strong>Available:</strong> <span style="color: <?= $is_out_of_stock ? '#dc3545' : '#28a745' ?>;"><?= number_format($available_quantity, 1) ?> <?= strtoupper($product['Unit']) ?></span>
                            </div>
                        </div>
                        <div>
            ...
            <!-- Order Form -->
            <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); <?= $is_out_of_stock ? 'opacity: 0.6; pointer-events: none;' : '' ?>">
                ...
                <form method="POST" id="orderForm" action="">
                    ...
                    <div style="margin-bottom: 25px;">
                        ...
                        <input type="number" 
                               ...
                               required>
                        <small style="color: #666;">Maximum available: <?= number_format($available_quantity, 1) ?> <?= strtoupper($product['Unit']) ?></small>
                    </div>
                    ...
                    <div style="margin-bottom: 25px;">
                        ...
                        <textarea name="notes" 
                                  ...
                                  style="width: 100%; padding: 12px; border: 2px solid #ddd; border-radius: 6px; font-size: 16px; resize: vertical;"
                                  <?= $is_out_of_stock ? 'disabled' : '' ?>><?= htmlspecialchars($_POST['notes'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div style="text-align: center;">
                        <button type="submit" name="submit_order"
                                id="submitBtn"
                                style="background: <?= $is_out_of_stock ? '#ccc' : 'linear-gradient(135deg, #28a745 0%, #34ce57 100%)' ?>; 
                                       color: <?= $is_out_of_stock ? '#999' : 'white' ?>; 
                                       padding: 15px 40px; 
                                       border: none; 
                                       border-radius: 8px; 
                                       font-size: 16px; 
                                       font-weight: 600; 
                                       cursor: <?= $is_out_of_stock ? 'not-allowed' : 'pointer' ?>; 
                                       box-shadow: <?= $is_out_of_stock ? 'none' : '0 4px 15px rgba(40, 167, 69, 0.3)' ?>; 
                                       transition: all 0.3s ease;
                                       filter: <?= $is_out_of_stock ? 'blur(0.1px)' : 'none' ?>;"
                                onmouseover="<?= !$is_out_of_stock ? "this.style.transform = 'translateY(-2px)'; this.style.boxShadow = '0 6px 20px rgba(40, 167, 69, 0.4)'" : '' ?>"
                                onmouseout="<?= !$is_out_of_stock ? "this.style.transform = 'translateY(0)'; this.style.boxShadow = '0 4px 15px rgba(40, 167, 69, 0.3)'" : '' ?>"
                                <?= $is_out_of_stock ? 'disabled' : '' ?>>
                            <?= $is_out_of_stock ? '🔒 Out of Stock' : '🛒 Place Order' ?>
                        </button>
                    </div>
                </form>
            </div>
...
document.getElementById('orderForm').addEventListener('submit', function(e) {
    ...
    if (isOutOfStock) {
        e.preventDefault();
        alert('This product is out of stock. Please try again later.');
        return false;
    }
