...
if (isset($_POST['update_status'])) {
    ...
    $allowed_transitions = [
        ...
        'CON' => ['COM'],
        'COM' => [],
        'REJ' => ['CON']  // NOW CAN CONFIRM REJECTED ORDERS
    ];
    ...
    <div class="modal-content-large">
        ...
        <div class="modal-body">
            ...
            <div class="status-update-section" id="statusUpdateSection">
                ...
                <!-- Status Buttons -->
                <div id="statusButtons" style="display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px;"></div>
            </div>
        </div>
...
<script>
// Define status workflow - UPDATED TO ALLOW REJECTED -> CONFIRMED
const statusWorkflow = {
    ...
    'REJ': { 
        allowed: ['CON'],  // NOW CAN CONFIRM REJECTED ORDERS
        info: 'Rejected order - You can Confirm this order now'
    }
};
...
function viewOrder(order) {
    ...
    // BUILD BUTTONS INSTEAD OF DROPDOWN
    ...
    if (allowedStatuses.length > 0) {
        allowedStatuses.forEach(status => {
            ...
            // Set button class and text based on status
            if (status === 'CON') {
            ...
            btn.onclick = function(e) {
                updateOrderStatus(e, order.Order_id, currentStatus, status, btn);
            };
            statusButtonsDiv.appendChild(btn);
        });
        
        document.getElementById('statusUpdateSection').style.opacity = '1';
    } else {
        statusButtonsDiv.innerHTML = '<p style="color: #666;">No actions available for this order</p>';
...
function updateOrderStatus(event, orderId, currentStatus, newStatus, btn) {
    ...
    .then(data => {
        if (data.success) {
            ...
            btn.disabled = false;
            if (newStatus === 'CON') btn.textContent = 'Confirm Order';
            else if (newStatus === 'COM') btn.textContent = 'Mark as Completed';
            else if (newStatus === 'REJ') btn.textContent = 'Reject Order';
        }
    })
    .catch(error => {
